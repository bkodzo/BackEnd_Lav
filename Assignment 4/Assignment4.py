...
def edit_friend():
    ...
    if 0 <= index < len(friends):
        name = friends[index]
        print(f"\nEditing {name}:")
        personal_info[name]["age"] = int(
            input(f"Enter new age (current: {personal_info[name]['age']}): ")
        )
        personal_info[name]["favColours"] = input(
            f"Enter new favorite colors (comma-separated, current: {', '.join(personal_info[name]['favColours'])}): "
        ).split(",")
        personal_info[name]["Job"] = input(
            f"Enter new Job (current: {personal_info[name]['Job']}): "
        )
        personal_info[name]["city"] = input(
            f"Enter new city (current: {personal_info[name]['city']}): "
        )
        personal_info[name]["quote"] = input(
            f"Enter new quote (current: {personal_info[name]['quote']}): "
        )
...
while True:
    ...
    if choice == "1":
        ...
        print(
            "Invalid choice. Please try again, or are you waiting for Ghana-maybe time?"
        )
